...
export function loadSource(options) {
    ...
    if (!existsSync(rootPath))
        throw new ToolFailure("ROOT_NOT_FOUND", "The explicit root does not exist", 4);
    if (!existsSync(requestedPath))
        throw new ToolFailure("FILE_NOT_FOUND", "The selected file does not exist", 2);
    let realRoot;
    let resolvedPath;
    try {
    ...
    if (!before.isFile())
        throw new ToolFailure("FILE_NOT_FOUND", "The selected path is not a regular file", 2);
    ...
    if (bytes.includes(0))
        throw new ToolFailure("BINARY_INPUT", "Binary or NUL-containing input is not supported", 2);
    let source;
    try {
