...
import { checkPureScriptCfc, PAIRED_TAGS, BRANCH_TAGS, BODYLESS_TAGS } from "./lexer.js";
function meta(limits) {
    return { scope: "One explicitly selected local UTF-8 .cfm or .cfc file under an explicit root", limits };
}
function emptyEnvelope(status, limits, errors) {
...
function fitOutput(envelope, limits) {
    const serialized = JSON.stringify(envelope);
    if (Buffer.byteLength(serialized, "utf8") <= limits.max_output_bytes)
        return envelope;
    const fallback = emptyEnvelope("incomplete", limits, [{ code: "LIMIT_EXCEEDED", message: "The output limit was reached; result data was withheld" }]);
    if (Buffer.byteLength(JSON.stringify(fallback), "utf8") <= limits.max_output_bytes)
        return fallback;
...
export function capabilities(limitsInput) {
...
export function checkFile(options) {
    let limits;
    try {
