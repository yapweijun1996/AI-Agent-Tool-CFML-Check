...
**Ledger baseline:** implementation `0.1.1`, release-hardening commit pending
...
| CFML-024 | Decide and execute CLI package publication | Pending final readback | 0.1.1 package prepared; authenticated registry version/tarball/integrity readback required |
...
| CFML-026 | Add GitHub Agent Skill and CI workflow | Done | `skills/agent-cfml-check/SKILL.md` and `.github/workflows/ci.yml` added; c76b8be CI passed, release commit CI pending |
...
- Remaining release tasks are blocked by missing external evidence or final registry readback, not by the current 22 passing tests.
...
- The prior c76b8be CI run passed; the release-hardening commit requires its own remote CI run.
